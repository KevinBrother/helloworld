...
export class MediaStorageService {
  ...
  saveMediaFilesToSession(
    ...
    this.logger.log(
      `会话 ${sessionId} 保存了 ${mediaFiles.length} 个媒体文件，总计 ${uniqueFiles.length} 个`
    );
  }
  ...
  searchMediaFiles(
    ...
    return files.filter((file) => {
      ...
      if (
        query.extension &&
        file.extension.toLowerCase() !== query.extension.toLowerCase()
      ) {
        return false;
      }
      ...
      if (
        query.fileName &&
        !file.fileName.toLowerCase().includes(query.fileName.toLowerCase())
      ) {
        return false;
      }
      ...
      if (
        query.sourceUrl &&
        !file.sourceUrl.toLowerCase().includes(query.sourceUrl.toLowerCase())
      ) {
        return false;
      }
