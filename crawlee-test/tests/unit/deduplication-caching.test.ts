  ...
  describe('URL去重机制', () => {
    it('应该能够自动去除重复URL', async () => {
      await withMockServer(async (helper) => {
        ...
        expect(processedUrls.length).toBeGreaterThan(uniqueUrls.size || processedUrls.length); // 可能因为去重而处理更少的URL
        
        // 检查是否处理了不同的页面
        const hasMainPage = processedUrls.some(url => url === helper.getServerUrl('/'));
      ...
      await withMockServer(async (helper) => {
        const crawler = new PlaywrightCrawler({
          requestHandler: async ({ request }) => {
            const fingerprint = generateFingerprint(request.url, request.userData);
  ...
  describe('请求指纹去重', () => {
      ...
      await withMockServer(async (helper) => {
        const crawler = new PlaywrightCrawler({
          requestHandler: async ({ page, request }) => {
            ...
            const existingFingerprint = await fingerprintStore.getValue(contentHash) as any;
    ...
    it('应该能够实现复杂的去重规则', async () => {
      const deduplicationRules = {
        ...
        // 判断是否为重复内容
        isDuplicate: function(newContent: any, existingContents: any[], threshold: number = 0.8): any {
          ...
          for (const existing of existingContents) {
            ...
            if (normalizedUrl === existingNormalizedUrl) {
              return { isDuplicate: true, reason: 'identical_url', original: existing };
            }
            
            // 内容相似性检查
            const similarity = this.calculateSimilarity(newContent.content, existing.content);
            if (similarity >= threshold) {
              return { isDuplicate: true, reason: 'similar_content', similarity, original: existing };
            }
          }
      ...
      const results = newContents.map(content => {
        const result = deduplicationRules.isDuplicate(content, existingContents);
